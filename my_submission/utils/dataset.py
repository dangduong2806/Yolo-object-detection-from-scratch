...
class YOLODataset(Dataset):
    ...
    def _load_image_and_boxes_xyxy(self, index):
        ...
        for ann in anns:
            ...
            boxes_xyxy_cls.append([x1, y1, x2, y2, class_id])

        return image, boxes_xyxy_cls
    
    def _yolo_to_xyxy_pixel(self, boxes_yolo):
        boxes_xyxy_cls = []

        for class_id, x, y, w, h in boxes_yolo:
            x1 = (x - w / 2.0) * self.image_size
            y1 = (y - h / 2.0) * self.image_size
            x2 = (x + w / 2.0) * self.image_size
            y2 = (y + h / 2.0) * self.image_size
            ...
            boxes_xyxy_cls.append([x1, y1, x2, y2, int(class_id)])

        return boxes_xyxy_cls
    
    def _load_mosaic(self, index):
        indices = [index]
        indices.extend(random.choices(range(len(self.images)), k=3))
        ...
        for idx, (offset_x, offset_y) in zip(indices, placements):
            image, boxes_xyxy_cls = self._load_image_and_boxes_xyxy(idx)

            # Dùng letterbox sẵn có để giữ tỉ lệ bbox đúng.
            image_lb, boxes_yolo = self._letterbox(
                image=image,
                boxes_xyxy_cls=boxes_xyxy_cls,
            )

            image_small = image_lb.resize((half, half), Image.BILINEAR)
            mosaic_image.paste(image_small, (offset_x, offset_y))

            boxes_lb_xyxy = self._yolo_to_xyxy_pixel(boxes_yolo)

            for x1, y1, x2, y2, class_id in boxes_lb_xyxy:
                x1 = x1 * 0.5 + offset_x
                y1 = y1 * 0.5 + offset_y
                x2 = x2 * 0.5 + offset_x
                y2 = y2 * 0.5 + offset_y
                ...
                mosaic_boxes.append([x1, y1, x2, y2, class_id])
    ...
    def __getitem__(self, index):
        ...
        if use_mosaic:
            image, boxes_xyxy_cls = self._load_mosaic(index)
        else:
            image, boxes_xyxy_cls = self._load_image_and_boxes_xyxy(index)
