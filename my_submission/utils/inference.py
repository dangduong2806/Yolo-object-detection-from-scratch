...
def apply_class_conf_thresholds(detections, class_names, class_conf_thresholds):
    ...
    for det in detections:
        ...
        threshold = class_conf_thresholds.get(class_name, 0.0)
...
def run_inference(
    ...
    if postprocess_config.get("enabled", False):
        detections = apply_class_conf_thresholds(
            ...
            class_conf_thresholds=postprocess_config.get("class_conf_thresholds", {}),
        )
    ...
    if postprocess_config.get("enabled", False):
        detections = limit_detections(
            detections=detections,
            max_detections=postprocess_config.get("max_detections_per_image", None),
        )
