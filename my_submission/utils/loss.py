...
class YOLOv3Loss(nn.Module):
    ...
    def _focal_bce_with_logits(self, pred, target):
        if pred.numel() == 0:
            return pred.sum() * 0.0
        ...
        focal_weight = (1.0 - p_t).pow(self.obj_focal_gamma)

        if self.obj_focal_alpha is not None:
            alpha_t = (
                self.obj_focal_alpha * target
                + (1.0 - self.obj_focal_alpha) * (1.0 - target)
            )
            focal_weight = alpha_t * focal_weight

        return (focal_weight * bce).mean()
    
    def _objectness_loss(self, pred, target):
        if self.obj_focal_gamma > 0:
            return self._focal_bce_with_logits(pred, target)

        return self._bce_with_logits(pred, target)
    ...
    def forward(self, predictions, target, anchors):
        ...
        #     target=noobj_target
        # )
        noobj_loss = self._objectness_loss(
            pred=noobj_pred,
            target=noobj_target
        )
